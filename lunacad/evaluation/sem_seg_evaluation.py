...
class SemSegEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            if sem_seg.dim() == 3:
                # Multi-channel logits: prefer argmax over classes.
                if sem_seg.shape[0] == 1:
                    output = (sem_seg.squeeze(0) >= self.sem_threshold).to(self._cpu_device)
                else:
                    output = sem_seg.argmax(dim=0).to(self._cpu_device)
            elif sem_seg.dim() == 2:
                # Single-channel probability/logit map.
                output = (sem_seg >= self.sem_threshold).to(self._cpu_device)
            else:
                raise ValueError(
                    f"Unexpected sem_seg shape {tuple(sem_seg.shape)} for {input.get('file_name')}"
                )
